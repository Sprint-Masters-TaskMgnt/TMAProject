    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(CreateUserDTO model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!emailResult)
                    {
                        TempData["Message"] += " However, we were unable to send the welcome email.";
                    }
        ...
        private async Task<bool> SendEmailAsync(string toEmail, string subject, string body)
        {
            try
            {
                var message = new MailMessage
                {
                    From = new MailAddress("samalaabhinaya2606@gmail.com"), // Use the email address from your config
                    Subject = subject,
                    Body = body,
                    IsBodyHtml = true // Change to true if you want to send HTML emails
                };
                message.To.Add(new MailAddress(toEmail));

                using (var smtpClient = new SmtpClient())
                {
                    // Settings are automatically fetched from web.config <system.net> section
                    await smtpClient.SendMailAsync(message);
                }
